...
<jsp:include page="../header.jsp"></jsp:include>
	<div class="row"> 
		<div class="col-md-9 wow fadeInDown" data-wow-duration="0.5s" style="width: 80%">
			<div>
				<!-- Nav tabs -->
				<ul class="nav nav-tabs" role="tablist">
			    	...
			    	<li role="presentation" ><a href="<%=basePath %>LostFound/lostFound_frontAdd.jsp">添加失物招领</a></li>
				</ul>
			  	<!-- Tab panes -->
				    					...
				    					<% 
 										...
 										<tr>
 											...
 											<td>
 												...
 												<a href="#" onclick="lostFoundEdit('<%=lostFound.getLostFoundId() %>');"><i class="fa fa-pencil fa-fw"></i>编辑</a>&nbsp;
 												<a href="#" onclick="lostFoundDelete('<%=lostFound.getLostFoundId() %>','<%=lostFound.getGoodsPhoto() %>');" ><i class="fa fa-trash-o fa-fw"></i>删除</a>
 											</td> 
 										</tr>
 										<%}%>
				    				</table>
						                	...
						                	<li>
												<div class="col-lg-3">
												    <div class="input-group">
												      <input type="text" id="pageValue" class="form-control" placeholder="跳转到指定页面">
	...
	<div class="col-md-3 wow fadeInRight" style="width: 20%">
    ...
    <div class="modal-content">
      ...
      <div class="modal-body" style="height:450px; overflow: auto;">
		  ...
		  <div class="form-group">
		   ...
		   <div class="form-group">
		  	 <label for="lostFound_goodsPhoto_edit" class="col-md-3 text-right">物品照片:</label>
		  	 <div class="col-md-9">
			    <img  class="img-responsive" id="lostFound_goodsPhotoImg" border="0px"/><br/>
			    <input type="hidden" id="lostFound_goodsPhoto" name="lostFound.goodsPhoto"/>
		  ...
		  <div class="form-group">
		    ...
		    <div class="form-group">
		  	 ...
		  	 <div class="col-md-9">
                <div class="input-group date lostFound_addTime_edit col-md-12" data-link-field="lostFound_addTime_edit">
                    <input class="form-control" id="lostFound_addTime_edit" name="lostFound.addTime" size="16" type="text" value="" placeholder="请选择捡得时间" readonly>
                    <span class="input-group-addon"><span class="glyphicon glyphicon-remove"></span></span>
                    <span class="input-group-addon"><span class="glyphicon glyphicon-calendar"></span></span>
                </div>
		  	 </div>
	...
	$("#goodsPhotoFile").change(function(){
        ...
        if (window.FileReader)
        {
            ...
            reader.onloadend = function (e) {
                $("#lostFound_goodsPhotoImg").attr("src",e.target.result);
            };
        }
	});
});
	...
	$.ajax({
		...
		success : function (lostFound, response, status) {
			if (lostFound) {
				...
				$("#lostFound_goodsPhotoImg").val(lostFound.pickUpTime);
				$("#lostFound_goodsPhoto").val(lostFound.goodsPhoto);
				$("#lostFound_goodsPhotoImg").attr("src", basePath +lostFound.goodsPhoto);
...
/*删除失物招领信息*/
function lostFoundDelete(lostFoundId,goodsPhoto) {
	if(confirm("确认删除这个记录")) {
		$.ajax({
			type : "POST",
...
$(function(){
	//验证寻物启事添加表单字段
	$('#lostFoundEditForm').bootstrapValidator({
		feedbackIcons: {
	...
	$('.lostFound_pickUpTime_edit').datetimepicker({
		...
		$('#lostFoundEditForm').data('bootstrapValidator').updateStatus('lostFound.pickUpTime', 'NOT_VALIDATED',null).validateField('lostFound.pickUpTime');
	});
	
	//发布时间组件
	$('.lostFound_addTime_edit').datetimepicker({
		language:  'zh-CN',  //显示语言
		format: 'yyyy-mm-dd hh:ii:ss',
		weekStart: 1,
		todayBtn:  1,
		...
		//下面这行代码解决日期组件改变日期后不验证的问题
		$('#lostFoundEditForm').data('bootstrapValidator').updateStatus('lostFound.addTime', 'NOT_VALIDATED',null).validateField('lostFound.addTime');
	});
})
