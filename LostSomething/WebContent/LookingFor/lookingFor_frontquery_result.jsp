...
<jsp:include page="../header.jsp"></jsp:include>
	<div class="col-md-9 wow fadeInLeft">
		<ul class="breadcrumb">
  			...
  			<a class="pull-right" href="<%=basePath %>LookingFor/lookingFor_frontAdd.jsp">添加寻物启事</a>
		</ul>
		<div class="row">
			<%
				/*计算起始序号*/
			...
			<div class="col-md-3 bottom15" <%=clearLeft %>>
			  <a  href="<%=basePath  %>LookingFor/<%=lookingFor.getLookingForId() %>/frontshow"><img class="img-responsive" src="<%=basePath%><%=lookingFor.getGoodsPhoto()%>" /></a>
			     <div class="showFields">
			     	...
			     	<div class="field">
	            		学生学号:<%=lookingFor.getUserObj().getUser_name() %>
			     	</div>
			     	<div class="field">
			        ...
			        <a class="btn btn-primary top5" onclick="lookingForDelete('<%=lookingFor.getLookingForId() %>','<%=lookingFor.getGoodsPhoto()%>');">删除</a>
			     </div>
			</div>
			<%  } %>
    ...
    <div class="modal-content">
      ...
      <div class="modal-body" style="height:450px; overflow: auto;">
		 ...
		 <div class="form-group">
		  	 ...
		  	 <div class="col-md-9">
                <div class="input-group date lookingFor_addTime_edit col-md-12" data-link-field="lookingFor_addTime_edit">
                    <input class="form-control" id="lookingFor_addTime_edit" name="lookingFor.addTime" size="16" type="text" value="" placeholder="请选择发布时间" readonly>
                    <span class="input-group-addon"><span class="glyphicon glyphicon-remove"></span></span>
                    <span class="input-group-addon"><span class="glyphicon glyphicon-calendar"></span></span>
                </div>
		  	 </div>
...
/*删除寻物启事信息*/
function lookingForDelete(lookingForId,goodsPhoto) {
	if(confirm("确认删除这个记录")) {
		$.ajax({
			type : "POST",
...
$(function(){
	//验证寻物启事添加表单字段
	$('#lookingForEditForm').bootstrapValidator({
		feedbackIcons: {
	...
	$('.lookingFor_lostTime_edit').datetimepicker({
		...
		$('#lookingForEditForm').data('bootstrapValidator').updateStatus('lookingFor.lostTime', 'NOT_VALIDATED',null).validateField('lookingFor.lostTime');
	});
	
	//发布时间组件
	$('.lookingFor_addTime_edit').datetimepicker({
		language:  'zh-CN',  //显示语言
		format: 'yyyy-mm-dd hh:ii:ss',
		weekStart: 1,
		todayBtn:  1,
		...
		//下面这行代码解决日期组件改变日期后不验证的问题
		$('#lookingForEditForm').data('bootstrapValidator').updateStatus('lookingFor.addTime', 'NOT_VALIDATED',null).validateField('lookingFor.addTime');
	});
})
