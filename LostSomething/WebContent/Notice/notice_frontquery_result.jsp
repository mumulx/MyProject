...
<%@ page import="com.mumulx.entity.Notice" %>
<%
		...
		<div class="col-md-9 wow fadeInDown" data-wow-duration="0.5s">
			<div>
				...
				<ul class="nav nav-tabs" role="tablist">
			    	...
			    	<li role="presentation" class="active"><a href="#noticeListPanel" aria-controls="noticeListPanel" role="tab" data-toggle="tab">站内通知列表</a></li>
			    	<li role="presentation" ><a href="<%=basePath %>Notice/notice_frontAdd.jsp" >添加站内通知</a></li>
				</ul>
			  	<!-- Tab panes -->
				    		...
				    		<div class="row">
				    			<div class="col-md-12 top5">
				    				...
				    				<table class="table table-condensed table-hover">
				    					<tr class="success bold"><td>序号</td><td>通知id</td><td>标题</td><td>发布时间</td><td>操作</td></tr>
				    					<% 
				    						/*计算起始序号*/
				    	            		...
				    	            		for(int i=0;i<noticeList.size();i++) {
					    	            		...
					    	            		Notice notice = noticeList.get(i); //获取到站内通知对象
 										%>
 										<tr>
 											<td><%=currentIndex %></td>
 											<td><%=notice.getNoticeId() %></td>
 											<td><%=notice.getTitle() %></td>
 											<td><%=notice.getAddTime() %></td>
 											<td>
 												<a href="<%=basePath  %>Notice/<%=notice.getNoticeId() %>/frontshow"><i class="fa fa-info"></i>&nbsp;查看</a>&nbsp;
 												<a href="#" onclick="noticeEdit('<%=notice.getNoticeId() %>');" ><i class="fa fa-pencil fa-fw"></i>编辑</a>&nbsp;
 												<a href="#" onclick="noticeDelete('<%=notice.getNoticeId() %>');" ><i class="fa fa-trash-o fa-fw"></i>删除</a>
 											</td> 
 										</tr>
 										<%}%>
				    				</table>
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        ...
        <h4 class="modal-title"><i class="fa fa-edit"></i>&nbsp;站内通知信息编辑</h4>
      </div>
      <div class="modal-body" style="height:450px; overflow: scroll;">
		  ...
		  <div class="form-group">
			 <label for="notice_noticeId_edit" class="col-md-3 text-right">通知id:</label>
			 <div class="col-md-9"> 
			 	<input type="text" id="notice_noticeId_edit" name="notice.noticeId" class="form-control" placeholder="请输入通知id" readOnly>
			 </div>
		  </div> 
		  <div class="form-group">
		  ...
		  <div class="form-group">
		  	 <label for="notice_addTime_edit" class="col-md-3 text-right">发布时间:</label>
		  	 <div class="col-md-9">
                <div class="input-group date notice_addTime_edit col-md-12" data-link-field="notice_addTime_edit">
                    <input class="form-control" id="notice_addTime_edit" name="notice.addTime" size="16" type="text" value="" placeholder="请选择发布时间" readonly>
	...
	$.ajax({
		...
		success : function (notice, response, status) {
			if (notice) {
				...
				$("#notice_addTime_edit").val(notice.addTime);
...
/*ajax方式提交站内通知信息表单给服务器端修改*/
function ajaxNoticeModify() {
	//提交之前先验证表单
	$("#noticeEditForm").data('bootstrapValidator').validate();
	if(!$("#noticeEditForm").data('bootstrapValidator').isValid()){
		return;
	}
	$.ajax({
...
$(function(){
	...
	//表扬时间组件
	$('.notice_addTime_edit').datetimepicker({
		language:  'zh-CN',  //显示语言
		...
		$('#noticeEditForm').data('bootstrapValidator').updateStatus('notice.addTime', 'NOT_VALIDATED',null).validateField('notice.addTime');
	});
