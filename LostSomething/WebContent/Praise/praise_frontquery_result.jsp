		...
		<div class="col-md-9 wow fadeInDown" data-wow-duration="0.5s">
			<div>
				...
				<ul class="nav nav-tabs" role="tablist">
			    	...
			    	<li role="presentation" ><a href="<%=basePath %>Praise/praise_frontAdd.jsp" >添加表扬</a></li>
				</ul>
			  	<!-- Tab panes -->
				    					...
				    					<% 
 										...
 										<tr>
 											...
 											<td>
 												...
 												<a href="#" onclick="praiseEdit('<%=praise.getPraiseId() %>');" ><i class="fa fa-pencil fa-fw"></i>编辑</a>&nbsp;
 												<a href="#" onclick="praiseDelete('<%=praise.getPraiseId() %>');" ><i class="fa fa-trash-o fa-fw"></i>删除</a>
 											</td> 
 										</tr>
 										<%}%>
				    				</table>
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      <div class="modal-body" style="height:450px; overflow: scroll;">
		  ...
		  <div class="form-group">
		  	 <label for="praise_lostFoundObj_lostFoundId_edit" class="col-md-3 text-right">失物招领Id:</label>
		  	 <div class="col-md-9">
		  ...
		  <div class="form-group">
		  	 <label for="praise_lostFoundObj_title_edit" class="col-md-3 text-right">招领信息:</label>
		  	 <div class="col-md-9">
			    <input type="text" id="praise_lostFoundObj_title_edit" name="praise.lostFoundObj.title" class="form-control" readonly>
			 </div>
		  </div>
		  <div class="form-group">
...
$(document).ready(function(){
	...
	$("#praise_lostFoundObj_lostFoundId_edit").change(function(){
		var title=$(this).find("option:selected").attr("lostfound_title")
		$("#praise_lostFoundObj_title_edit").val(title);
	});
});
			...
			if (praise) {
				...
				$.ajax({
					...
					success: function(lostFounds,response,status) { 
		        		...
		        		var title=$("#praise_lostFoundObj_lostFoundId_edit").find("option:selected").attr("lostfound_title")
		        		$("#praise_lostFoundObj_title_edit").val(title);
					}
				});
...
function ajaxPraiseModify() {
	//提交之前先验证表单
	$("#praiseEditForm").data('bootstrapValidator').validate();
	if(!$("#praiseEditForm").data('bootstrapValidator').isValid()){
		return;
	}
	$.ajax({
...
$(function(){
	//验证认领添加表单字段
	$('#praiseEditForm').bootstrapValidator({
		feedbackIcons: {
	...
	$('.praise_addTime_edit').datetimepicker({
		...
		$('#praiseEditForm').data('bootstrapValidator').updateStatus('praise.addTime', 'NOT_VALIDATED',null).validateField('praise.addTime');
	});
})
</script>
</body>
