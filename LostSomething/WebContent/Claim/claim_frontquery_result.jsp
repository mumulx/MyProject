		...
		<div class="col-md-9 wow fadeInDown" data-wow-duration="0.5s">
			<div>
				...
				<ul class="nav nav-tabs" role="tablist">
			    	...
			    	<li role="presentation" ><a href="<%=basePath %>Claim/claim_frontAdd.jsp" >添加认领</a></li>
				</ul>
			  	<!-- Tab panes -->
				    		...
				    		<div class="row">
				    			<div class="col-md-12 top5">
				    				...
				    				<table class="table table-condensed table-hover">
				    					<tr class="success bold"><td>序号</td><td>认领id</td><td>失物招领id</td><td>招领信息</td><td>认领人</td><td>认领时间</td><td>发布时间</td><td>操作</td></tr>
				    					<% 
				    						/*计算起始序号*/
 										...
 										<tr>
 											...
 											<td><%=claim.getLostFoundObj().getLostFoundId() %></td>
 											...
 											<td>
 												...
 												<a href="#" onclick="claimEdit('<%=claim.getClaimId() %>');" ><i class="fa fa-pencil fa-fw"></i>编辑</a>&nbsp;
 												<a href="#" onclick="claimDelete('<%=claim.getClaimId() %>');"><i class="fa fa-trash-o fa-fw"></i>删除</a>
 											</td> 
 										</tr>
 										<%}%>
				    				</table>
					            ...
					            <div class="col-md-12">
						            <nav class="pull-left">
						                <ul class="pagination">
						                  	...
						                  	<li>
												<div class="col-lg-3">
												    <div class="input-group">
												      <input type="text" id="changePageValue" class="form-control" placeholder="跳转到指定页面">
												      <span class="input-group-btn">
												        <button class="btn btn-default" type="button" onclick="changepage(<%=totalPage%>)">Go</button>
												      </span>
												    </div>
												</div>
											</li>
											<li>
												<div class="col-lg-4">
									            	<div class="pull-right" style="line-height:34px;" >共有<%=recordNumber %>条记录，当前第 <%=currentPage %>/<%=totalPage %> 页</div>
												</div>
											</li>
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      <div class="modal-body" style="height:450px; overflow: auto;">
		  ...
		  <div class="form-group">
		  	 <label for="claim_lostFoundObj_lostFoundId_edit" class="col-md-3 text-right">失物招领Id:</label>
		  	 <div class="col-md-9">
			    <select id="claim_lostFoundObj_lostFoundId_edit" name="claim.lostFoundObj.lostFoundId" class="form-control">
			    </select>
			    <input type="hidden" id = "lfoOldId" name = "lfoOldId">
		  	 </div>
		  </div>
		  
		  <div class="form-group">
		  	 <label for="claim_lostFoundObj_title_edit" class="col-md-3 text-right">招领信息:</label>
		  	 <div class="col-md-9">
			    <input type="text" id="claim_lostFoundObj_title_edit" name="claim.lostFoundObj.title" class="form-control" placeholder="请输入招领信息" readonly>
			 </div>
		  </div>
		  
		  <div class="form-group">
		  	 <label for="claim_personName_edit" class="col-md-3 text-right">认领人学号:</label>
		  	 <div class="col-md-9">
			    <input type="text" id="claim_personName_edit" name="claim.personName"  class="form-control" placeholder="请输入认领人学号">
			 </div>
		  </div>
		  ...
		  <div class="form-group">
		   ...
		   <div class="form-group">
		  	 ...
		  	 <div class="col-md-9">
                <div class="input-group date claim_addTime_edit col-md-12" data-link-field="claim_addTime_edit">
                    <input class="form-control" id="claim_addTime_edit" name="claim.addTime" size="16" type="text" value="" placeholder="请选择认领时间" readonly>
                    <span class="input-group-addon"><span class="glyphicon glyphicon-remove"></span></span>
                    <span class="input-group-addon"><span class="glyphicon glyphicon-calendar"></span></span>
                </div>
		  	 </div>
...
$(document).ready(function(){
	...
	$("#claim_lostFoundObj_lostFoundId_edit").change(function(){
		var title=$(this).find("option:selected").attr("lostfound_title")
		$("#claim_lostFoundObj_title_edit").val(title);
	});
});
			...
			if (claim) {
				...
				$.ajax({
					...
					success: function(lostFounds,response,status) { 
		        		...
		        		$("#lfoOldId").val(claim.lostFoundObjPri);
		        		
		        		var title=$("#claim_lostFoundObj_lostFoundId_edit").find("option:selected").attr("lostfound_title")
		        		$("#claim_lostFoundObj_title_edit").val(title);
		        		//$("#claim_lostFoundObj_title_edit").val();
...
function ajaxClaimModify() {
	//提交之前先验证表单
	$("#claimEditForm").data('bootstrapValidator').validate();
	if(!$("#claimEditForm").data('bootstrapValidator').isValid()){
		return;
	}
	$.ajax({
...
$(function(){
	//验证认领添加表单字段
	$('#claimEditForm').bootstrapValidator({
		feedbackIcons: {
	...
	$('.claim_claimTime_edit').datetimepicker({
		...
		$('#claimEditForm').data('bootstrapValidator').updateStatus('claim.claimTime', 'NOT_VALIDATED',null).validateField('claim.claimTime');
	});
	//发布时间组件
	$('.claim_addTime_edit').datetimepicker({
		language:  'zh-CN',  //显示语言
		format: 'yyyy-mm-dd hh:ii:ss',
		weekStart: 1,
		todayBtn:  1,
		...
		//下面这行代码解决日期组件改变日期后不验证的问题
		$('#claimEditForm').data('bootstrapValidator').updateStatus('claim.addTime', 'NOT_VALIDATED',null).validateField('claim.addTime');
	});
