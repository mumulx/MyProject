...
import com.mumulx.util.ExportExcelUtil;

//Praise管理控制层
@Controller
@RequestMapping("/Praise")
public class PraiseController extends HelpDevController {
	...
	public void list(@ModelAttribute("lostFoundObj") LostFound lostFoundObj,String title,String addTime,Integer page,Integer rows, Model model, HttpServletRequest request,HttpServletResponse response) throws Exception {
		...
		if(rows != 0)praiseService.getHelpDev().setRows(rows);
	    ...
	    /*获取到总的页码数目*/
	    int totalPage = praiseService.getHelpDev().getTotalPage();
	    /*当前查询条件下总记录数*/
	    int recordNumber = praiseService.getHelpDev().getRecordNumber();
